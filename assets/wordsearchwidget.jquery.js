...
(function ($, undefined) {
    ...
    function Grid() {
        ...
        this.put = function (row, col, word) {
            ...
            var populator = eval("new " + eval("this.directions[" + Math.floor(Math.random() * this.directions.length) + "]") + "Populator(row,col,word, this)");
            ...
            for (var i = 0; i < this.size(); i++) {
                for (var j = 0; j < this.size(); j++) {
                    if (this.cells[i][j].isUnwritten()) {
                        this.cells[i][j].value = String.fromCharCode(Math.floor('А'.charCodeAt() + Math.random() * 31));
                    }
                }
            }
